...
# function to check for files matching a glob
has_files () {
...
if !(test -d $AISHELLROOT); then
    tar -xvf $AISHELL -C $(dirname $AISHELL)
...
if has_files "$AISHELLROOT/kws/hs/*.bin"; then
    echo "Skipping utterance hs for train (already exists)."
else
...
for split in "dev" "test"; do
    if has_files "$AISHELLROOT/hotword/$split/hs/*.bin"; then
        echo "Skipping utterance hs for $split (already exists)."
    else
...
if has_files "$AISHELLROOT/kws/keywords-audios/natural/*.wav" || has_files "$AISHELLROOT/kws/keywords-audios/natural/*.mp3"; then
    echo "Skipping natural keyword audios for train (already exists)."
else
...
for split in "dev" "test"; do
    if has_files "$AISHELLROOT/hotword/$split/keywords-audios/natural/*.wav" || has_files "$AISHELLROOT/hotword/$split/keywords-audios/natural/*.mp3"; then
        echo "Skipping natural keyword audios for $split (already exists)."
    else
...
if has_files "$AISHELLROOT/kws/keywords-hs/natural/*.bin"; then
    echo "Skipping natural keyword hs for train (already exists)."
else
...
for split in "dev" "test"; do
    if has_files "$AISHELLROOT/hotword/$split/keywords-hs/natural/*.bin"; then
        echo "Skipping natural keyword hs for $split (already exists)."
    else
...
# use PaddleSpeech to generate synthetic audios for the keywords
echo "Generating synthetic audios for the keywords..."
if has_files "$AISHELLROOT/kws/keywords-audios/tts/*.wav" || has_files "$AISHELLROOT/kws/keywords-audios/tts/*.mp3"; then
    echo "Skipping TTS keyword audios for train (already exists)."
else
    python3 ../../src/utils.py --tts -k "$AISHELLROOT/kws/keywords_voice.txt" -t "$AISHELLROOT/kws/keywords-audios/tts/" -l "zh-CN"
fi
for split in "dev" "test"; do
    if has_files "$AISHELLROOT/hotword/$split/keywords-audios/tts/*.wav" || has_files "$AISHELLROOT/hotword/$split/keywords-audios/tts/*.mp3"; then
        echo "Skipping TTS keyword audios for $split (already exists)."
    else
...
if has_files "$AISHELLROOT/kws/keywords-hs/tts/*.bin"; then
    echo "Skipping TTS keyword hs for train (already exists)."
else
...
for split in "dev" "test"; do
    if has_files "$AISHELLROOT/hotword/$split/keywords-hs/tts/*.bin"; then
        echo "Skipping TTS keyword hs for $split (already exists)."
    else

...
import sys
import time
from prepare import ROOT,OUT,MIG,MODEL,ADAPTER,DATA,dump,norm,sha
...
def evaluate(path):
    ...
    refs={line.split(maxsplit=1)[0]:norm(line.split(maxsplit=1)[1]) for line in (DATA/'dev/uttid').read_text(encoding='utf-8-sig').splitlines() if line.strip()}
    designated=[]

...
class BridgeBTests(unittest.TestCase):
    ...
    def test_actionable_prompt_v2(self):
        audit = {}
        output = bridge.build_user_prompt(self.record, args(include_missing_kws_hotwords=True), audit)
        self.assertIn('retrieved_hotwords_without_candidate_match=', output)
        self.assertNotIn('- unsupported_kws_hotwords=', output)
        self.assertIn('不必要求 N-best 已包含该热词', output)
        self.assertIn('不要为了使用热词而新增内容', output)
        self.assertIn('你没有获得原始音频或局部声学证据', output)
        self.assertEqual(audit['mode'], 'B_missing_kws_prompt_v2')

#!/usr/bin/env python
"""Bridge CB-SenseVoice evidence JSONL to COVO/Qwen text-rewrite inference."""
...
SYSTEM_MESSAGE = (
    "你是一个保守的中文 ASR 后纠错器。根据 CB-SenseVoice 输出、N-best 候选、"
...
INSTRUCTION = (
    "任务：融合 CB-SenseVoice 的最终输出、N-best 候选和 KWS 热词证据进行中文 ASR 后纠错。"
    "优先保留 CB-SenseVoice 输出；只有当 N-best、拼音或高置信热词共同支持时才修改。"
    ...
    "除非多个可信候选和上下文都明确支持替换。"
    "热词证据来自 CB-SenseVoice/KWS，不是参考答案；它用于保护专名/领域词，"
    "但不能因为热词分数高就强行插入无上下文支持的词。"
    "如果证据不足，保持 ASR top-1 不变。"
)

SELECTOR_INSTRUCTION = (
    "任务：从 CB-SenseVoice 的 ASR top-1、N-best 候选、拼音和 KWS 热词证据中选择最可信的完整中文转写。"
    ...
    "热词证据来自 CB-SenseVoice/KWS，不是参考答案；不要因为热词分数高就强行插入无上下文支持的词。"
    "输出应尽量等于某个高质量 N-best 候选；不要为了保守而复制 top-1。"
    "只有所有候选都存在明显局部错字时，才在最佳候选基础上做小幅修正。"
)

CONTENT_SELECTOR_INSTRUCTION = (
    "任务：从 CB-SenseVoice 的 ASR top-1、N-best 候选、拼音和 KWS 热词证据中选择主体内容最可信的完整中文转写。"
    ...
    "热词证据来自 CB-SenseVoice/KWS，不是参考答案；不要强行插入无上下文支持的词，也不要把已正确出现的领域词改成同音常见词。"
    "输出应尽量等于某个高质量 N-best 候选；只有候选存在明显局部错字时才做小幅修正。"
)

SPOKEN_SELECTOR_INSTRUCTION = (
    "任务：从 CB-SenseVoice 的 ASR top-1、N-best 候选、拼音和 KWS 热词证据中选择最可信的课堂口语转写。"
...
def build_user_prompt(record: Dict[str, Any], args: argparse.Namespace) -> str:
    ...
    if hotword_lines:
        lines.append("CB-SenseVoice hotword evidence (predicted, not gold):")
...
def add_prepare_args(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--input", required=True, help="CB-SenseVoice evidence JSONL from CBW_EVIDENCE_OUT")
...
def parse_args() -> argparse.Namespace:
    ...
    prepare = subparsers.add_parser("prepare", help="Convert CB-SenseVoice evidence to Qwen messages")
    ...
    run.add_argument("--covo-dir", default=str(Path(__file__).resolve().parents[2] / "covo"))
    run.add_argument("--python", default=sys.executable)
    run.add_argument("--model-name-or-path", default=str(Path(__file__).resolve().parents[2] / "models" / "Qwen3.5-4B"))
    run.add_argument("--adapter-path", default=str(Path(__file__).resolve().parents[2] / "covo" / "outputs" / "qwen35_text_rewrite_hardneg_dropout_lora_2epoch"))

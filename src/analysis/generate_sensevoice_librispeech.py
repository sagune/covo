#!/usr/bin/env python3
"""Generate English LibriSpeech CTC N-best hypotheses with SenseVoice."""
...
def normalize_text(text: str) -> str:
    ...
    value = TAG_RE.sub(" ", value).replace("▁", " ").lower()
...
def encode_audio(model, tokenizer, frontend, audio_path: str, device: torch.device, use_itn: bool):
    ...
    language_id = model.lid_dict.get("en", model.lid_dict["auto"])
...
def main() -> int:
    ...
    parser.add_argument("--token-topk", type=int, default=32)
    ...
    # IDs 3-15 are common English pieces ("the", "to", "and", ...), not
    # control tokens. The Chinese generator's broad low-ID filter must not be
    # reused here or it silently removes function words from every hypothesis.
    excluded = {1, 2} | set(range(25000, vocab_size))
    ...
    with args.input.open("r", encoding="utf-8") as handle:
        for row_index, line in enumerate(handle):
            if not line.strip() or row_index % args.num_shards != args.shard_index:
                continue
            row = json.loads(line)
            if str(row.get("id", "")) not in completed:
                rows.append(row)
    if args.limit:
    ...
    with args.output.open("a", encoding="utf-8") as writer:
        for row in rows:
            ...
            if args.progress_every and written % args.progress_every == 0:
                ...
                print(json.dumps({"written": written, "rate": written / elapsed}), flush=True)
    print(json.dumps({"output": str(args.output), "written": written}))
    return 0

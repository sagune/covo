#!/usr/bin/env python3
"""Final acceptance sweep for the front-end reranker.

One rule is being validated: the acoustic override may reorder / lengthen the
front-end's own hypothesis but must never shorten it (the forced-CTC score is a
*total* log-likelihood, so an unconstrained argmax deletes characters -- the whole
ST-CMDS regression).  Here we score that rule against the shipped one at both the
original and the relaxed admission, on every dataset/admission pair.
"""
import argparse
import json
import sys
...
def main():
    ...
    print("# %s   rows=%d unscored=%d chars=%d mentions=%d" % (a.label, len(cache), unscored, chars, men))

    def sel(key, floor):
        def f(old, sub, hw):
            ok = [c for c in sub if len(c["text"]) >= len(old) + floor] if floor else sub
            return max(ok, key=key)["text"] if ok else old
        return f

    policies = [
        ("identity (front-end top-1)", lambda o, s, h: o),
        ("shipped: max(exact,ctc)", sel(lambda c: (c["exact"], c["ctc"]), None)),
        ("FIX: max(exact,ctc), no-shorten", sel(lambda c: (c["exact"], c["ctc"]), 0)),
        ("FIX: max(exact,ctc_ln), no-shorten", sel(lambda c: (c["exact"], c["ctc_ln"]), 0)),
    ]

    print("%-34s %8s %9s %8s %8s %6s %6s" % ("policy", "CER%", "dCER pp", "recall%", "drec pp", "spur", "chg%"))
    base = None
    for name, fn in policies:
        ...
        if base is None:
            base = (cer, rcl)
        print("%-34s %8.4f %+9.4f %8.2f %+8.2f %6d %6.1f" % (

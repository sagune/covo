#!/usr/bin/env bash
# Second sweep on the same three pools: does length-normalising the forced-CTC
# score (and/or freezing the hotword set) remove the ST-CMDS regression?
set -uo pipefail
WS=/root/autodl-tmp
R="$WS/.dsh_checks/rerank"
PY="$WS/great/bin/python"
SW="$WS/.dsh_checks/rerank_policy_sweep2.py"
LOG="$R/policy_sweep2.log"
: > "$LOG"
...
echo "[$(date -Is)] POLICY_SWEEP2_DONE" | tee -a "$LOG"
touch "$R/POLICY_SWEEP2_DONE"

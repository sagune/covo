...
ADAPTER_ROOT="$ROOT/outputs/qwen35_9b_aishell_hardneg_dropout_2epoch_20260815"
DATA="$ROOT/data/processed/chinesehp_aishell1"
OUT="$ROOT/outputs/qwen35_9b_aishell_checkpoint_selection_20260817"
LOGS=/root/autodl-tmp/src/logs

mkdir -p "$OUT/screen" "$OUT/full_dev" "$OUT/test" "$LOGS"
cd "$ROOT"

DEV="$DATA/dev_text_rewrite_hardneg.qwen.jsonl"
TEST="$DATA/test_text_rewrite_hardneg.qwen.jsonl"
SCREEN="$OUT/dev_screen_1024.jsonl"

if [[ ! -s "$DEV" ]]; then
  "$PY" scripts/prepare_text_rewrite_data.py \
    --input "$DATA/dev.jsonl" --output "$DEV" \
    --evidence-mode nbest --max-nbest 10 --max-pinyin 5 \
    --include-pinyin --max-hard-negatives 4
fi

if [[ ! -s "$TEST" ]]; then
  "$PY" scripts/prepare_text_rewrite_data.py \
    --input "$DATA/test.jsonl" --output "$TEST" \
    --evidence-mode nbest --max-nbest 10 --max-pinyin 5 \
    --include-pinyin --max-hard-negatives 4
fi

# Uniform coverage across the ordered 14,326-row dev set.
awk 'NR % 14 == 1' "$DEV" > "$SCREEN"
echo "[$(date '+%F %T')] screen_rows=$(wc -l < "$SCREEN")"

checkpoints=(3750 7500 11250 15000 18750 22500 26250 30000 30022)
...
for path in Path(sys.argv[1]).glob("checkpoint-*.summary.json"):
    ...
    checkpoint = path.name.removesuffix(".summary.json")
    rows.append((float(metrics["cer"]), int(metrics["worsened_samples"]), checkpoint))
rows.sort()
...
"$PY" - "$best_checkpoint" "$test_summary" "$OUT/final_summary.json" <<'PY'
import datetime
import json
import sys
from pathlib import Path

...
class _ContextBiasASR(pl.LightningModule):
    @staticmethod
    ...
    def _load_kws_model_from_ckpt(ckpt_path: str) -> KWSModel:
        ...
        """
        ckpt = torch.load(ckpt_path, map_location="cpu")
        kws_kwargs = _ContextBiasASR._infer_kws_model_kwargs_from_ckpt(ckpt_path)
    ...
    def __init__(
        ...
        if self.hparams.asr_backend != "sensevoice":
            raise ValueError(
                "CB-SenseVoice only supports asr_backend='sensevoice'; "
                "the legacy CB-SenseVoice route has been retired."
            )
    ...
    def test_step(self, batch, batch_idx):  
        ...
        # get predictions from the CB-SenseVoice for the given setting
...
class CBSenseVoice(_ContextBiasASR):

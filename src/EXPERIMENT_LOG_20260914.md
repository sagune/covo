...
| 13:07–13:09 | 桥接层 CPU 预演 + 接口审计 | 0 | 顺带把接口不一致从 4.6% 降到 0.5% |
...
### E38 · 接口一致性审计
- **先纠正我自己的一个错误**：`compact_evidence` **默认为 True**，`format_candidates` 只在 `if not compact` 时渲染 → 所谓 "candidate scores" 块**在我们的所有臂里根本没被渲染**（计数 0）。原先 §7.9 的机制描述有误，真正的不一致发生在 **n-best 块内部**。
- 真实观察（row id=535，ST-CMDS）：
  ```
  ASR top-1: 二中说这一周周末我约你去网吧
  1. 二中说这一周周末我约你去网吧 | rank=12 score=2.802 asr=-7.342
  2. 二中说啊,这周周末我约你去网吧。 | rank=1  score=3.800 asr=-0.370
  ```
  即 COVO 被要求考虑的第 1 行，其自带元数据把它排在第 12 位。
- 发生率（winner 原 rank > 8 / 平均 rank 位移）：出厂重排 ST-CMDS **234/5130 (4.6%)** / 5.96 → 修复重排 **26/5130 (0.5%)** / 2.65；AISHELL 8/1334 (0.6%)、THCHS-30 8/2495 (0.3%)。**修复顺带把接口不一致降低约 9 倍。**
